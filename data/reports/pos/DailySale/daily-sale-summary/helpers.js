...
function netPrice(SalePrice, Discount) {
  var sanitize = function(val) {
    if (val === undefined || val === null) return 0;
    if (typeof val === 'number') return val;
    var sanitized = val.toString().replace(/[^\d.-]/g, '');
    var parsed = parseFloat(sanitized);
    return isNaN(parsed) ? 0 : parsed;
  };

  var sale = sanitize(SalePrice);
  var disc = sanitize(Discount);
  var net = sale - disc;
...
function sumField(items, field) {
  ...
  items.forEach(function (item) {
    if (item && item[field] !== undefined && item[field] !== null) {
      total += sanitize(item[field]);
    }
  });
  return total;
}

function sumNetPrice(items) {
  if (!Array.isArray(items)) return 0;
  ...
  items.forEach(function (item) {
    if (item) {
      var sale = sanitize(item.SalePrice);
      var discount = sanitize(item.Discount);
      total += (sale - discount);
    }
  });
  return total;
